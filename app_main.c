...
*///629 và 544 là 2 dòng cần thiết
#include <stdio.h>
#include <stdint.h>
#include <stddef.h>
#include <string.h>
...
static const char *TANG = "MQTTWS_EXAMPLE";
char myData[128] = "vuong";
char compareV[10] = "vuong";
int flag = 1;
bool stop_mqtt_client = false;
bool stop_ble = false;
...
static uint8_t raw_adv_data[] = {
        ...
        0x03, 0x03, 0xFF, 0x00,
        /* device name */
        0x0E, 0x09, 'L', 'O', 'P', '2', 'N', 'H', 'O', 'M', '3', 'L', 'A', 'B', '4'
};
...
const uint8_t char_value[8] = {0x00, 0x11, 0x22, 0x33, 0x00, 0x11, 0x22, 0x33};
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            if (memcmp(myData, compareV, strlen(myData)))
                {
                    memcpy((char*)char_value, myData, strlen(myData));
                }
            //ESP_LOGE(EXAMPLE_TAG, "ESP_GATTS_READ_EVT, handle=0x%d, offset=%d", param->read.handle, param->read.offset);
            if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->read.handle) {//read.handle = 42
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle){ 
                        ...
                        memcpy(myData, (char*)param->write.value, param->write.len);
...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ESP_LOGI(TANG, "MQTT_EVENT_CONNECTED");
            msg_id = esp_mqtt_client_subscribe(client, "topic0", 0);
            ESP_LOGI(TANG, "sent subscribe successful, msg_id=%d", msg_id);
        ...
        case MQTT_EVENT_DISCONNECTED:
            ESP_LOGI(TANG, "MQTT_EVENT_DISCONNECTED");
            break;
        case MQTT_EVENT_SUBSCRIBED:
            //đẩy giá trị của myData đã lưu từ BLE lên MQTT thông qua topic 1
            ESP_LOGI(TANG, "MQTT_EVENT_SUBSCRIBED, msg_id=%d", event->msg_id);
            msg_id = esp_mqtt_client_publish(client, "topic1", myData, 0, 1, 0);
            ESP_LOGI(TANG, "sent publish successful, msg_id=%d", msg_id);
            break;
        case MQTT_EVENT_UNSUBSCRIBED:
            ESP_LOGI(TANG, "MQTT_EVENT_UNSUBSCRIBED, msg_id=%d", event->msg_id);
            break;
        case MQTT_EVENT_PUBLISHED:
            ESP_LOGI(TANG, "MQTT_EVENT_PUBLISHED, msg_id=%d", event->msg_id);
            break;
        case MQTT_EVENT_DATA:
            //Trong trường hợp nhận thấy việc ghi dữ liệu từ bất kỳ topic nào, thì sẽ lưu giá trị đã ghi vào myData
            memset(myData, 0, strlen(myData));
            ESP_LOGI(TANG, "MQTT_EVENT_DATA");
            printf("TOPIC=%.*s\r\n", event->topic_len, event->topic);
            printf("DATA=%.*s\r\n", event->data_len, event->data);
            memcpy(myData, event->data, event->data_len);
            flag = 1;
            printf("Flagmqtt=%d\n", flag);
            if (!stop_mqtt_client) {
                stop_mqtt_client = true;
            }
            break;
        case MQTT_EVENT_ERROR:
            ESP_LOGI(TANG, "MQTT_EVENT_ERROR");
            break;
        default:
            ESP_LOGI(TANG, "Other event id:%d", event->event_id);
            break;
    }
    return ESP_OK;
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ESP_LOGD(TANG, "Event dispatched from event loop base=%s, event_id=%d", base, event_id);
...
void app_main(void)
{
    ...
    while(1)
    {
        switch(flag)
        {
            ...
            //MQTT
                stop_ble = false;
                ESP_LOGI(TANG, "[APP] Startup..");
                ESP_LOGI(TANG, "[APP] Free memory: %d bytes", esp_get_free_heap_size());
                ESP_LOGI(TANG, "[APP] IDF version: %s", esp_get_idf_version());
                ...
                mqtt_app_start();
                while (!stop_mqtt_client) {
                        vTaskDelay(pdMS_TO_TICKS(100));  // Delay for 100 milliseconds
                    }
                esp_wifi_disconnect();
                esp_wifi_stop();
                esp_wifi_deinit();
                esp_mqtt_client_unsubscribe(myclient, "topic0");
                esp_mqtt_client_unsubscribe(myclient, "topic1");
